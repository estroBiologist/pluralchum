...
export function generateBioComponents(bio) {
  ...
  const scrollerContainer = BdApi.React.createElement(
    'div',
    { className: scrollerClass + ' ' + thinClass, style: { overflow: 'hidden scroll', paddingRight: '8px' } },
    markupContainer,
  );
...
export function generatePopoutBioComponents(bio) {
  ...
  const scrollerContainer = BdApi.React.createElement(
    'div',
    { className: thinClass, style: { overflow: 'hidden scroll', 'max-height': '30vh' } },
    markupContainer,
  );
  return scrollerContainer;
}
